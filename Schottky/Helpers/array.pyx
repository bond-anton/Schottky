...
@boundscheck(False)
@wraparound(False)
cdef double[:] gradient1d(double[:] y, double[:] x, double scale=1.0):
    cdef:
        int i, n = x.shape[0]
    ...
    b = (dx1 + dx2) / (dx1 * dx2)
    c = - dx1 / (dx2 * (dx1 + dx2))
    result[0] = (a * y[0] + b * y[1] + c * y[2]) * scale
    dx1 = x[n - 2] - x[n - 3]
    dx2 = x[n - 1] - x[n - 2]
    a = dx2 / (dx1 * (dx1 + dx2))
    b = - (dx2 + dx1) / (dx1 * dx2)
    c = (2.0 * dx2 + dx1) / (dx2 * (dx1 + dx2))
    result[n - 1] = (a * y[n - 3] + b * y[n - 2] + c * y[n - 1]) * scale
    for i in range(1, n - 1):
        result[i] = ((y[i + 1] - y[i - 1]) / (x[i + 1] - x[i - 1])) * scale
    return result
